...
from base import application, _Brick, Brick, lazy
...
class Random(Brick):
    ...
    """
    seed_rng = numpy.random.RandomState(1)
...
class MLP(Sequence, Initializable, Feedforward):
    ...
    def __init__(self, activations, dims, **kwargs):
        ...
        application_methods = []
        for entity in [x for t in zip(self.linear_transformations, activations) for x in t]:
            if entity is None:
                continue
